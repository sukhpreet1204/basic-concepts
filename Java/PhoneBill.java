 ...
 *		   (iii) All calls to the phone number with the longest total call duration are free. 
                 In the event that two phone numbers have the same total call duration use the one that is smaller.
*/
...
public class PhoneBill {
    ...
    public int totalCost(String log) {
        ...
        // create a hashmap to store the phoneNumber and the total phone call duration
        // for it
        ...
        for (String record : logEntries) {
            ...
            // if we already have an entry for this phone number then add this total
            // duration to the existing one else just add it
        ...
        /*
         * Find the phone call with the maximum duration in event of a tie use the one
         * with least numerical value (NOTE: I think this is redundant as we don't need
         * to bother with the tie break given that they all have the same total duration
         * and it's that value we will use to calculate the total bill)
         */
        int maxDuration = 0;
        ...
        /*
         * Finally we can evaluate the total cost of the phone bill. Rules: (i) For a
         * phone call less than 5 minutes long charge 3 cents per second (ii) For a
         * phone call 5 minutes and longer charge 150 cents per started minute, so for a
         * call of 5 minutes 2 seconds would be charged as a 6 minute call i.e. 6 * 150
         * = 900 cents (iii) All calls to the phone number with the longest total call
         * duration are free. In the event that two phone numbers have the same total
         * call duration use the one
         */
        int totalCost = 0;

        for (Map.Entry<String, Integer> entry : phoneNumberDetails.entrySet()) {
            if (entry.getKey().equals(maxPhoneNumber))
                continue;
